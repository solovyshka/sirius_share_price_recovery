...
from Modeling.residual_models import (
    ArimaResidualModel,
...
def run_all_demos():
    ...
    tech_feature = technical_indicator_feature(
        ticker=ticker, indicator_columns=["SMA_20", "EMA_12", "MACD", "RSI", "OBV"]
    )
    ...
    catboost_factory = lambda: CatBoostResidualModel(n_lags=lags_for_boosting)

    autoarima_exog_factory = lambda: PerColumnAutoArimaExogForecaster(
    ...
    catboost_exog_factory = lambda: CatBoostExogForecaster(n_lags=lags_for_boosting)
    ...
    evaluate_demo(
        ...
        ForecastPipeline(
            ...
            decomposition_fn=lambda s: stl_decomposition(
                s,
                trend_forecaster=trend_forecast_patchtst(),
            ),
            model_factory=arima_factory,
        ),
        raw_df,
    ...
    evaluate_demo(
        "STL + logistic trend + CatBoost residual + CatBoost exog forecast",
        ForecastPipeline(
            ticker=ticker,
            feature_builders=[rolling_feat, fracdiff_feat, tech_feature],
            decomposition_fn=lambda s: stl_decomposition(
            ...
            model_factory=catboost_factory,
            exog_forecast_factory=catboost_exog_factory,
        ),
        raw_df,
        test_size,
        plot_tail,
    ...
    #         ),
    #         model_factory=arima_factory,
    #         exog_forecast_factory=autoarima_exog_factory,
    #     ),
    #     raw_df,
    #     test_size,
    #     plot_tail,
