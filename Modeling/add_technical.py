...
class TechnicalIndicators:
    ...
    def load_data(self, ticker):
        try:
            close = pd.read_csv(f'{self.data_folder}CLOSE.csv', index_col=1, parse_dates=True)[ticker].rename('close')
            high = pd.read_csv(f'{self.data_folder}HIGH.csv', index_col=1, parse_dates=True)[ticker].rename('high')
            low = pd.read_csv(f'{self.data_folder}LOW.csv', index_col=1, parse_dates=True)[ticker].rename('low')
            open_ = pd.read_csv(f'{self.data_folder}OPEN.csv', index_col=1, parse_dates=True)[ticker].rename('open')
            volume = pd.read_csv(f'{self.data_folder}VOLUME.csv', index_col=1, parse_dates=True)[ticker].rename('volume')
    ...
    def calculate_all_indicators(self, data, ticker):
        ...
        def calc_adx(high, low, close, window=14): # [adx, +di, -di, dx]
            ...
            for i in range(window + 1, n): # Smoothed = Previous Smoothed - (Previous Smoothed/window) + Current
                smoothed_tr[i] = smoothed_tr[i-1] - (smoothed_tr[i-1] / window) + true_range.iloc[i]
                smoothed_plus_dm[i] = smoothed_plus_dm[i-1] - (smoothed_plus_dm[i-1] / window) + plus_dm_series.iloc[i]
                smoothed_minus_dm[i] = smoothed_minus_dm[i-1] - (smoothed_minus_dm[i-1] / window) + minus_dm_series.iloc[i]
        ...
        def calc_stoch_osc(high, low, close, k_period=14, d_period=3,
                           smooth_k=1, ma_type='sma'):# [%K, %D, raw_k, signal]
            n = len(close)
            ...
            for i in range(k_period - 1, n):
                ...
                if period_high != period_low:
                    raw_k[i] = 100 * (current_close - period_low) / (period_high - period_low)
                else:
                    raw_k[i] = 0.0
            ...
            for i in range(k_period + max(smooth_k, d_period) - 2, n):
                if ma_type == 'sma':
                    ...
                    if i == k_period + max(smooth_k, d_period) - 2:
                        ...
                        percent_d[i] = percent_k[i] * alpha + percent_d[i-1] * (1 - alpha)
            ...
            signal_series = pd.Series(stoch_signal, index=close.index, name='Stoch_Signal')
        ...
        df['Upper_Channel_20'] = df['high'].shift(1).rolling(window=20, min_periods=1).max()
        df['Lower_Channel_20'] = df['low'].shift(1).rolling(window=20, min_periods=1).min()
        
        df['ticker'] = ticker
        
        return df
    
    def get_indicators(self, ticker, timestamp=None, window=None): # (timestamp-window, timestamp]
        if ticker not in self.indicators_data:
            data = self.load_data(ticker)
            if data is not None and len(data) > 50:
                indicators_df = self.calculate_all_indicators(data, ticker)
        ...
        if timestamp not in df.index:
            ...
            if len(available_dates) > 0:
                ...
                print(f"Указанный timestamp не найден. Используется ближайший доступный: {timestamp}")
            else:
