...
class TechnicalIndicators:
    ...
    def calculate_all_indicators(self, data, ticker):
        ...
        def calc_stoch_osc(high, low, close, k_period=14, d_period=3, smooth_k=1, ma_type='sma'):
            n = len(close)
        ...
        df['Upper_Channel_20'] = df['high'].rolling(window=20, min_periods=1).max()
        df['Lower_Channel_20'] = df['low'].rolling(window=20, min_periods=1).min()
        df['ticker'] = ticker
        return df.shift(1)
